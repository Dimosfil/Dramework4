...
- Branch naming: use `codex/` for agent-created branches unless the user asks
  for another prefix.
- Generated files policy: keep Unity-generated folders, IDE projects, user
  settings, logs, caches, local databases, and build outputs out of commits.
  Commit package source, manifests, metadata, tests, and reviewable agent
  instruction files when they are part of the requested change.
